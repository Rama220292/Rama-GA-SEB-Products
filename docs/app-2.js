...
const moveWasteToTab = () => {
    ...
    if (pile.length !== 0){
        inSequence = selectedCard.rank + 1 === pile[pile.length - 1].rank && selectedCard.color !== pile[pile.length - 1].color    
    } 
...
const moveTabToTab = () => {
    console.log("moveTabToTab is working.")
    const pile = masterTableau[selectedPile.pilekey]

    //  if card is King and Pile empty or if card rank is one lower, and color different, then shift
    const cardKingTableauEmpty = selectedCard.rank === 13 && pile.length === 0
    let inSequence = false
    if (pile.length !== 0){
        inSequence = selectedCard.rank + 1 === pile[pile.length - 1].rank && selectedCard.color !== pile[pile.length - 1].color    
...
const newCardElement = (card) => {
    ...
    const suitSymbols = {                                   // Generated by ChatGPT
    spades: "♠",
    hearts: "♥",
    clubs: "♣",
    diamonds: "♦"
    ...
    if (card.faceDown) {
        ...
        newCardDiv.textContent = card.character + " " + suitSymbols[card.suit] // Generated by ChatGPT
    }
    return newCardDiv
}
...
const renderTableaus = () => {
    ...
    Object.entries(masterTableau).forEach(([key,value], index) => {
        ...
        for (const item of value) {
            ...
            if (!item.faceDown) {
                if (item.color === "red") {
                addCard.classList.add("red-card")
                } else {
                addCard.classList.add("black-card")
                }
            }
...
const renderWastePile = () => {
    ...
    if (wastePile.length > 0) {
        ...
        const cardElement = newCardElement(card)
        ...
        cardElement.addEventListener('click', (event) => {
            ...
            selectCardFunction(card, 'waste', 'waste-pile')
        })
...
const renderFtn = () => {
    ...
    for (let suit in foundationPiles){
        ...
        if (pile.length !== 0) {
            ...
            ftnElement.appendChild(cardElement)
